...
try {
  ...
  // Test 8: daemon start is idempotent — a second start in a home that already
  // has a live daemon no-ops instead of spawning a competing daemon (#733).
  {
    console.log("Test 8: daemon start no-ops when a daemon already runs (#733)");
    const idempotentPort = 10000 + Math.floor(Math.random() * 50000);
    const first = await daemonCommand(["start", "--port", String(idempotentPort)]);
    assert.strictEqual(first.exitCode, 0, "first start should succeed");

    // Wait until the daemon process is alive (it has acquired its pid lock) so the
    // second start sees it. The pid-lock guard keys off the live process, which
    // status reports as "running" or (while the socket is still warming up)
    // "unresponsive" — both mean the process owns the home.
    let running = false;
    for (let attempt = 0; attempt < 30; attempt++) {
      const status = await daemonCommand(["status", "--json"]);
      const localDaemon = JSON.parse(status.stdout).localDaemon;
      if (localDaemon === "running" || localDaemon === "unresponsive") {
        running = true;
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 200));
    }
    assert(running, "daemon should be alive before the second start");

    const second = await daemonCommand(["start", "--port", String(idempotentPort)]);
    assert.strictEqual(second.exitCode, 0, "second start should no-op with exit 0");
    assert(
      second.stdout.toLowerCase().includes("already running"),
      "second start should report the daemon is already running instead of spawning a competitor",
    );

    const cleanup = await daemonCommand(["stop", "--force"]);
    assert.strictEqual(cleanup.exitCode, 0, "cleanup stop should succeed after idempotent start");
    console.log("✓ daemon start no-ops when a daemon already runs\n");
  }
} finally {
