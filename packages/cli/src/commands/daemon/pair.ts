    ...
    if (client) {
      ...
      if (supportsDaemonStatusRpc) {
        try {
          ...
          await client.close().catch(() => {}); // intentional: best-effort teardown after reading the pairing offer
          outputPairingResult(
      ...
      await client.close().catch(() => {}); // intentional: best-effort teardown of the pairing probe connection
    }
  }
