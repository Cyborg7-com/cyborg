...
export async function runRestartCommand(
  ...
  // Infer the target home (#665) when no --home was given, so restart doesn't
  // bounce the wrong/empty default while a daemon runs in a custom home.
  const target = resolveTargetHomeForCommand(startOptions.home);
  if (target.home) startOptions.home = target.home;
