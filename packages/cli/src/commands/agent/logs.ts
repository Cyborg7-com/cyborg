  ...
  try {
    ...
    if (options.follow) {
      if (options.tail !== undefined && parseTailCount(options.tail) === undefined) {
        ...
        await client.close().catch(() => {}); // intentional: best-effort teardown before exit; the usage error is already printed
        process.exit(1);
      }
    ...
    await client.close().catch(() => {}); // intentional: best-effort teardown before exit; the failure is already printed
    process.exit(1);
  }
}
