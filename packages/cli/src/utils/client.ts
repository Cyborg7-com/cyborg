...
export function resolveDaemonTarget(host: string): DaemonTarget {
  ...
  // Full WebSocket URL — used AS-IS. This is how the CLI targets the cloud RELAY
  // (e.g. `--host wss://relay.cyborg7.com/api/ws`) instead of a daemon's bare
  // `/ws`: combined with `--workspace`/`--daemon <id>` + `--token`, the cyborg
  // terminal commands (create/ls/send-keys/capture) then route THROUGH the relay
  // to a remote daemon — controlling another machine's terminal. A daemon's own
  // `/ws` path keeps working via the bare-host branches below.
  if (trimmed.startsWith("ws://") || trimmed.startsWith("wss://")) {
    return { type: "tcp", url: trimmed };
  }
  if (
    trimmed.startsWith("unix://") ||
    trimmed.startsWith("pipe://") ||
...
async function tryConnectHost(
  ...
  try {
    ...
    await client.close().catch(() => {}); // intentional: best-effort teardown; the connect error is returned to the caller
    return { error };
  }
}
...
async function connectViaRelayOffer(
  ...
  try {
    ...
    await client.close().catch(() => {}); // intentional: best-effort teardown; the connect error is rethrown below
