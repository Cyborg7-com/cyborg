...
let nodePtyModule: typeof pty | null = null;
// Latched so the win32 in-$INSTDIR fallback warn (see loadNodePty) fires at most
// once per process, not on every terminal spawn.
...
// Lazy node-pty load (replacing the former static `import * as pty`). On a packaged
// Windows desktop the install-dir node-pty's native images (conpty.node / pty.node)
// get memory-mapped and a surviving holder — the detached pty-host, a Defender scan
// — keeps a lock that fails the NSIS auto-update uninstall (code 2). The desktop
// main process copies the loadable node-pty subtree to a stable external dir and
// exports its path via CYBORG7_NATIVE_DIR (see desktop-cyborg/native-relocate.ts);
// resolve from there so NOTHING under $INSTDIR is loaded. Off Windows / when unset /
// when the external copy is absent, resolve node-pty normally. Cached after first
// load. This is the single node-pty load site for BOTH the daemon worker and the
// detached pty-host (both call createTerminal below).
function loadNodePty(): typeof pty {
  ...
  if (process.platform === "win32" && externalDir) {
    // Require the `.complete` marker (written LAST by native-relocate.ts) so a
    // crash-interrupted, partial copy is never loaded; absent → in-$INSTDIR copy.
    const marker = join(externalDir, ".complete");
    const externalEntry = join(externalDir, "node-pty");
    if (existsSync(marker) && existsSync(externalEntry)) {
      nodePtyModule = require(externalEntry) as typeof pty;
      return nodePtyModule;
    }
  }
  ...
  // ":2" lock on the NEXT update (see desktop-cyborg/native-relocate.ts). Make it
  // observable. Warn at most once per process (this is the single node-pty load
  // site for BOTH the daemon worker and the detached pty-host). Off-win32 there is
  // no relocation, so stay silent.
  ...
  nodePtyModule = require("node-pty") as typeof pty;
  return nodePtyModule;
}
...
export async function createTerminal(options: CreateTerminalOptions): Promise<TerminalSession> {
  ...
  const ptyProcess = loadNodePty().spawn(spawnCommand, spawnArgs, {
    name: "xterm-256color",
    cols,
    rows,
    cwd,
