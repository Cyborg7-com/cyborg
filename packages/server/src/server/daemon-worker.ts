...
function bootstrapFromEnvironment(): BootstrapResult {
  try {
    ...
    // Wire Logfire observability before anything else logs. Token-gated no-op
    // when LOGFIRE_TOKEN is absent. attachPinoBridge makes every existing
    // logger.error/fatal (incl. the uncaughtException/unhandledRejection handlers
    // and the bootstrap fatal) flow to Logfire automatically — no call-site changes.
    const version = resolveDaemonVersion();
    const environment = process.env.CYBORG_ENV ?? process.env.NODE_ENV ?? "dev";
    configureObservability({ platform: "daemon", version, environment });
...
async function main() {
  ...
  const installExitHook = () => {
    ...
    void shutdownPromise.then(async (exitCode) => {
      // Drain pending spans to Logfire before the process dies (no-op when
      // observability is disabled / never throws).
...
// why the daemon crashed can be lost. Also drain Logfire spans in the same
// window so the crash's telemetry isn't lost (no-op when disabled).
