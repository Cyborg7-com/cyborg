...
export class VoiceAssistantWebSocketServer {
  ...
  private cyborgDispatcher?: SessionOptions["cyborgDispatcher"];
  private cyborgDaemonMode: "solo" | "connected" = "solo";
  private cyborgPairingOffer: {
    serverId: string;
    daemonPublicKeyB64: string;
    relay: { endpoint: string; useTls?: boolean };
  } | null = null;
  private readonly cyborgWorkspaceClients = new Map<string, Set<WebSocketLike>>();
  private readonly cyborgUserClients = new Map<string, Set<WebSocketLike>>();
  ...
  setCyborgDispatcher(dispatcher: SessionOptions["cyborgDispatcher"]): void {
    this.cyborgDispatcher = dispatcher;
  }

  setCyborgDaemonMode(mode: "solo" | "connected"): void {
    this.cyborgDaemonMode = mode;
  }

  setCyborgPairingOffer(offer: {
    serverId: string;
    daemonPublicKeyB64: string;
    relay: { endpoint: string; useTls?: boolean };
  }): void {
    this.cyborgPairingOffer = offer;
  }

  getCyborgPairingOffer(): {
    serverId: string;
    daemonPublicKeyB64: string;
    relay: { endpoint: string; useTls?: boolean };
  } | null {
    return this.cyborgPairingOffer;
  }

  broadcastCyborgMessage(msg: unknown): void {
    const payload = JSON.stringify({ type: "session", message: msg });
  ...
  broadcastCyborgMessageToUser(userId: string, msg: unknown): void {
    const clients = this.cyborgUserClients.get(userId);
  ...
  broadcastCyborgMessageToWorkspace(workspaceId: string, msg: unknown): void {
    const clients = this.cyborgWorkspaceClients.get(workspaceId);
    if (!clients || clients.size === 0) return;
    const payload = JSON.stringify({ type: "session", message: msg });
    for (const ws of clients) {
      if (ws.readyState === 1) {
  ...
  registerCyborgWorkspaces(ws: WebSocketLike, workspaceIds: string[]): void {
    for (const wId of workspaceIds) {
      let set = this.cyborgWorkspaceClients.get(wId);
      if (!set) {
        set = new Set();
        this.cyborgWorkspaceClients.set(wId, set);
      }
      set.add(ws);
    }
  }

  registerCyborgUser(ws: WebSocketLike, userId: string): void {
    let set = this.cyborgUserClients.get(userId);
    if (!set) {
      set = new Set();
      this.cyborgUserClients.set(userId, set);
    }
    set.add(ws);
  }

  unregisterCyborgClient(ws: WebSocketLike): void {
    for (const [wId, set] of this.cyborgWorkspaceClients) {
      set.delete(ws);
      if (set.size === 0) this.cyborgWorkspaceClients.delete(wId);
    }
    for (const [uid, set] of this.cyborgUserClients) {
      set.delete(ws);
      if (set.size === 0) this.cyborgUserClients.delete(uid);
    }
  }
  ...
  private createSessionConnection(params: {
    ...
    const session = new Session({
      ...
      cyborgDispatcher: this.cyborgDispatcher,
      cyborgDaemonMode: this.cyborgDaemonMode,
      onCyborgAuth: (workspaceIds, userId) => {
        if (connection) {
          for (const s of connection.sockets) {
            this.registerCyborgWorkspaces(s, workspaceIds);
            if (userId) {
              this.registerCyborgUser(s, userId);
            }
          }
        }
      },
  ...
  private async detachSocket(
    ...
    this.unregisterCyborgClient(ws);
