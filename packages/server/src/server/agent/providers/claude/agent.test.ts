...
describe("ClaudeAgentClient binary resolution", () => {
  ...
  test("drops the global 'user' setting source for internal generations", async () => {
    // config.internal (cyborg slash /summarize + metadata one-shots) must NOT load
    // the user's GLOBAL ~/.claude settings/MCP/CLAUDE.md, which pollute a one-shot
    // result. Keep only the workspace-scoped project/local sources. Auth is
    // unaffected (credentials are not a setting source).
    const queryReturn = vi.fn();
    queryReturn.mockResolvedValue(undefined);
    const queryFactory = vi.fn(() => ({
      close: vi.fn(),
    ...
    const sources = queryFactory.mock.calls[0]?.[0].options.settingSources;
    expect(sources).toEqual(["project", "local"]);
