...
// INTERNAL one-shot generations (config.internal — the cyborg slash /summarize +
// metadata completers) must NOT load the user's GLOBAL "user" settings
// (~/.claude: settings.json system-prompt/MCP/hooks + ~/.claude/CLAUDE.md), which
// inject context that pollutes a one-shot text result (the Claude analogue of the
// Pi global-extension bug fixed in #345). Drop only "user"; keep the workspace-
// scoped "project"/"local" sources. Setting sources do NOT carry credentials, so
// auth is unaffected.
const CLAUDE_INTERNAL_SETTING_SOURCES: NonNullable<ClaudeOptions["settingSources"]> = [
  "project",
  "local",
];
...
class ClaudeAgentSession implements AgentSession {
  ...
  private async buildOptions(): Promise<ClaudeOptions> {
    ...
    const base: ClaudeOptions = {
      ...
      settingSources:
        this.config.internal === true ? CLAUDE_INTERNAL_SETTING_SOURCES : CLAUDE_SETTING_SOURCES,
      stderr: (data: string) => {
