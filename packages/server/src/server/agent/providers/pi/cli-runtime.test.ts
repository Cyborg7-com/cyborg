...
describe("PiCliRuntime", () => {
  ...
  test("launches an ephemeral (--no-session) Pi session when requested", async () => {
    const child = createPiChild();
    replyToCommands(child, () => ({}));
    const launches: PiRuntimeLaunch[] = [];
    const runtime = createRuntime(child, launches);

    await runtime.startSession({ cwd: "/workspace/project", ephemeral: true });
  ...
  test("does not pass --no-session for a normal (persisted) session", async () => {
    const child = createPiChild();
    replyToCommands(child, () => ({}));
    const launches: PiRuntimeLaunch[] = [];
    const runtime = createRuntime(child, launches);
  ...
  test("passes --no-extensions (discovery off) while keeping explicit -e extensions", async () => {
    const child = createPiChild();
    replyToCommands(child, () => ({}));
    const launches: PiRuntimeLaunch[] = [];
    const runtime = createRuntime(child, launches);
    ...
    await runtime.startSession({
      cwd: "/workspace/project",
      noExtensions: true,
      extensionPaths: ["/tmp/paseo-ext.ts"],
    });

    const argv = launches[0]?.argv ?? [];
    expect(argv).toContain("--no-extensions");
    // The explicit paseo extension is still loaded (discovery off ≠ -e off).
    expect(argv).toContain("--extension");
    expect(argv).toContain("/tmp/paseo-ext.ts");
  });

  test("does not pass --no-extensions for a normal session", async () => {
    const child = createPiChild();
    replyToCommands(child, () => ({}));
    const launches: PiRuntimeLaunch[] = [];
    const runtime = createRuntime(child, launches);
    ...
    expect(launches[0]?.argv).not.toContain("--no-extensions");
  });

  test("resuming an explicit session wins over ephemeral (no --no-session)", async () => {
    const child = createPiChild();
    replyToCommands(child, () => ({}));
    const launches: PiRuntimeLaunch[] = [];
    const runtime = createRuntime(child, launches);
    ...
    await runtime.startSession({
      cwd: "/workspace/project",
      session: "/tmp/sessions/sess-1.json",
      ephemeral: true,
    });

    expect(launches[0]?.argv).toContain("--session");
    expect(launches[0]?.argv).not.toContain("--no-session");
  });
