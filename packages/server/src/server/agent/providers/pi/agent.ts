...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    // Guard the pending result up front: if `prompt()` below rejects (disposed
    // session / timeout) the await throws before the try, and a later
    // process_exit would reject this orphan with no handler -> unhandledRejection.
    resultPromise.catch(() => {});
    ...
    try {
      ...
      // The Pi process can exit while this tree command is in flight;
      // handleProcessExit then rejects the pending result. Surface that as an
      // empty result instead of letting the rejection escape as an
      // unhandledRejection, which would crash the daemon worker.
      return undefined;
    }
  }
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    try {
      ...
      // The Pi process can exit (e.g. "Agent is already processing") while this
      // capture is in flight; handleProcessExit then rejects the pending result.
      // Treat that as an empty capture instead of letting the rejection escape as
      // an unhandledRejection, which would crash the daemon worker.
    }
  }
  ...
  async createSession(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        // persistSession=false → launch Pi with `--no-session` (its ephemeral
        // mode). Without it, an internal one-off generation (e.g. the slash
        // /summarize completer) shares Pi's session store with the user's live
        // session and the Pi RPC server rejects the concurrent prompt with
        // "Agent is already processing. Specify streamingBehavior…". An ephemeral
        // session is standalone and never persisted, so it can't collide.
        ephemeral: options?.persistSession === false,
        // INTERNAL generations (config.internal — the cyborg slash /summarize +
        // metadata completers) must NOT auto-discover the user's GLOBAL/project Pi
        // extensions: those inject context (e.g. a "Design Studio" extension emits
        // {"continue":true,"systemMessage":"…detected project context…"}) that
        // pollutes a one-shot text result. `--no-extensions` disables discovery
        // while the explicit paseoExtension (-e above) still loads, and the real
        // config dir/auth is untouched. User-facing agents are unaffected.
        noExtensions: config.internal === true,
      });
    } catch (error) {
      mcpConfig?.cleanup();
