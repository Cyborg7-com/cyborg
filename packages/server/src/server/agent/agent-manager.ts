...
export interface AgentManagerOptions {
  ...
  mcpBaseUrl?: string;
  // Cyborg7: base URL of the daemon's /mcp/cyborg7 endpoint. When set, a NON-cybo
  // workspace agent gets the cyborg7 task/schedule/channel tools injected at spawn
  // (mirrors mcpBaseUrl). Null on a pure-Paseo daemon (no cyborg tools).
...
export class AgentManager {
  ...
  private cyborg7McpBaseUrl: string | null;
  ...
  constructor(options: AgentManagerOptions) {
    ...
    this.cyborg7McpBaseUrl = options?.cyborg7McpBaseUrl ?? null;
  ...
  // Inject the daemon's MCP HTTP servers into a session config at spawn:
  //  - `paseo`: every agent (callerAgentId-scoped) — the existing behavior.
  //  - `cyborg7`: a NON-cybo workspace agent (a plain Claude/Codex session) so it
  //    gets the same task/schedule/channel tools a cybo gets. A cybo already
  //    receives its own cyborg7 server via config.mcpServers (see spawnCybo) and is
  //    tagged with a `cyboId` label, so it is excluded here — this never
  //    double-injects nor clobbers the cybo path. Requires a workspaceId (the
  //    cyborg7 tools are workspace-scoped) and the daemon's cyborg7 base URL.
  // Injected servers are merged BEFORE config.mcpServers so an explicit config
  // entry always wins (a cybo's own cyborg7 server, a caller-supplied override).
  private injectMcpServers(
    ...
    const cyborg7BaseUrl = this.cyborg7McpBaseUrl;
    const workspaceId = options?.workspaceId;
    const isCybo = options?.labels?.cyboId != null;
    if (
      cyborg7BaseUrl != null &&
      workspaceId != null &&
      !isCybo &&
      config.mcpServers?.cyborg7 == null
    ) {
      injected.cyborg7 = {
        type: "http",
        url: `${cyborg7BaseUrl}?workspaceId=${encodeURIComponent(workspaceId)}&agentId=${encodeURIComponent(resolvedAgentId)}`,
      };
    }
    if (Object.keys(injected).length === 0) {
      return config;
    }
    return { ...config, mcpServers: { ...injected, ...config.mcpServers } };
  }
  ...
  async createAgent(
    ...
    const injectedConfig = this.injectMcpServers(config, resolvedAgentId, options);
