...
export interface SessionOptions {
  ...
  // Cyborg7: collaborative workspace dispatcher
  cyborgDispatcher?: {
    dispatch(
      msg: { type: string; [key: string]: unknown },
      auth: unknown,
      emit: (msg: unknown) => void,
      setAuth?: (ctx: unknown) => void,
      subscribeWorkspace?: (workspaceId: string) => void,
    ): Promise<void> | undefined;
  };
  cyborgAuthContext?: unknown;
  cyborgDaemonMode?: "solo" | "connected";
  onCyborgAuth?: (workspaceIds: string[], userId: string | null) => void;
  daemonRuntimeConfig?: {
    listen: string | null;
    relay: {
      enabled: boolean;
...
export class Session {
  ...
  private readonly cyborgDispatcher: SessionOptions["cyborgDispatcher"];
  private readonly cyborgDaemonMode: "solo" | "connected";
  private cyborgAuthContext: SessionOptions["cyborgAuthContext"];
  private readonly onCyborgAuth: SessionOptions["onCyborgAuth"];
  ...
  constructor(options: SessionOptions) {
    ...
    this.cyborgDispatcher = cyborgDispatcher;
    this.cyborgDaemonMode = options.cyborgDaemonMode ?? "solo";
    this.cyborgAuthContext = cyborgAuthContext;
    this.onCyborgAuth = options.onCyborgAuth;
  ...
  private async dispatchInboundMessage(msg: SessionInboundMessage): Promise<void> {
    const promise =
      ...
      this.cyborgDispatcher?.dispatch(
        msg as unknown as { type: string; [key: string]: unknown },
        this.cyborgAuthContext,
        (m) => this.emit(m as SessionOutboundMessage),
        (ctx) => {
          this.cyborgAuthContext = ctx;
          const authCtx = ctx as { user?: { id: string }; workspaces?: { id: string }[] } | null;
          if (authCtx?.workspaces) {
            this.onCyborgAuth?.(
              authCtx.workspaces.map((w) => w.id),
              authCtx.user?.id ?? null,
            );
          }
        },
        (workspaceId: string) => {
          this.onCyborgAuth?.([workspaceId], null);
        },
      ) ??
    ...
    try {
      ...
      this.emit({
        ...
        payload: {
          ...
          providers,
          cyborgMode: this.cyborgDaemonMode,
        },
      });
    } catch (error) {
