<script lang="ts">
  // Slack mark (the official 4-color "hash" logo). Unlike GitHubIcon (a monochrome
  // mark that adapts to the theme via currentColor), Slack's logo IS intrinsically
  // multicolor, so the four brand hexes are baked into the paths on purpose — a brand
  // glyph is not re-themed, so these are deliberately NOT theme tokens. Used as the
  // brand glyph on the integrations card + the Slack detail page header.
